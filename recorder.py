...
def toggle_recording():
    ...
    if not recording:
        ...
        recording = True
        start_button.config(text="Stop Recording", bg="#FCFCFC", fg="black")
        audio_data = sd.rec(5 * sd.default.samplerate, samplerate=sd.default.samplerate, channels=2, dtype="int16")
        start_timer()
    else:
        # Stop recording
        recording = False
        start_button.config(text="Start Recording", bg="#FCFCFC",fg="black")
        sd.stop()
        sd.wait()
...
myimage = tk.Label(image=photo, bg="#FFFAF0")
...
label = tk.Label(root, text="Voice Recorder", font=custom_font, bg="#FFFAF0",fg="#3B3024")
label.place(relx=0.5, rely=0.02, anchor="n")

# Create the Save Audio button
save_button = tk.Button(root, text="Save Audio", font=(custom_font.actual()['family'], 20), bg="#FCE6C9", fg="black",\
                        command=save_audio, state=tk.DISABLED, width=13)
...
start_button = tk.Button(root, text="Start Recording",font=(custom_font.actual()['family'], 20), command=toggle_recording,\
                         bg="#FCE6C9",fg="black", width=13)
start_button.place(x=150,y=330)

# Create the recording timer label
timer_label = tk.Label(root, text="", font=(custom_font.actual()['family'], 32), bg="#FFFAF0", fg="#3B3024")
timer_label.place(x=50,y=413)
